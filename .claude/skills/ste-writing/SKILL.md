...
- No em dashes. STE itself does not ban them, but this project does. Use a period, a comma, or parentheses.
...
7. Any em dash? Replace it.

To check a file mechanically, run the bundled linter:

```
python3 .claude/skills/ste-writing/ste-lint.py path/to/draft.md
```

It reports violations per 100 words. Lower is cleaner. Lint a draft, apply the skill, then lint again. The delta between the two scores is the signal. The linter is a heuristic, not a certified STE checker.
